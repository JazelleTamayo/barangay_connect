<?php
// Barangay Connect – Captain Reports
// captain/reports.php
// FIXED: Match secretary reports UI — charts, CSV export, 4th stat card (collection total),
//        Payment Collection section, toggle chart/table, proper card-actions styling.
// ADDED: Completed transactions (released requests) table and CSV export.
...
// FIXED: Payment collection summary — Released requests with payments this month
    ...
    switch ($export) {
        case 'monthly':
            ...
            foreach ($monthlySummary as $r) {
                fputcsv($out, [$r['RequestType'], $r['Total'], $r['Released'], $r['Rejected'], $r['Cancelled'],
        ...
        case 'payments':
            ...
            foreach ($paymentSummary as $r) {
                fputcsv($out, [$r['RequestType'], $r['TotalReleased'],
        ...
        case 'staff':
            ...
            foreach ($staffPerformance as $r) {
                fputcsv($out, [$r['FullName'] ?? 'Unnamed', $r['Role'], $r['TotalProcessed'],
                    $r['AvgHours'] !== null ? number_format((float)$r['AvgHours'], 1) : '0.0']);
            }
            break;
        case 'pending':
            ...
            foreach ($weeklyPending as $r) {
                fputcsv($out, [$r['ReferenceNo'], $r['ResidentName'], $r['RequestType'], $r['Status'], (int)$r['HoursElapsed']]);
            }
            break;
        case 'demographics':
    ...
    <main class="main-content">
        ...
        <div class="page-body">
            ...
            <div class="stats-grid">
                ...
                <!-- FIXED: 4th stat card — monthly collection (was missing entirely) -->
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            <?php if (empty($monthlySummary)): ?>
                                <tr><td colspan="6" class="empty-row">No requests this month.</div></tr>
            ...
            <!-- FIXED: Payment Collection Summary (was completely missing in captain) -->
            <div class="card mt-4">
                <div class="card-header">
                    <div>
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            <?php if (empty($paymentSummary)): ?>
                                <tr><td colspan="5" class="empty-row">No payments recorded this month.</div></tr>
                        ...
                        <table class="data-table compact-table">
                            ...
                            <tbody>
                                <?php if (empty($complaintSummary)): ?>
                                    <tr><td colspan="3" class="empty-row">No complaints found.</div></tr>
                        ...
                        <table class="data-table compact-table">
                            ...
                            <tbody>
                                <?php if (empty($facilityUtilization)): ?>
                                    <tr><td colspan="3" class="empty-row">No facility data found.</div></tr>
                        ...
                        <table class="data-table compact-table">
                            ...
                            <tbody>
                                <?php if (empty($staffPerformance)): ?>
                                    <tr><td colspan="4" class="empty-row">No staff activity found.</div></tr>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        <?php if (empty($weeklyPending)): ?>
                            <tr><td colspan="5" class="empty-row">✅ No requests are past SLA.</div></tr>
                        <?php else: foreach ($weeklyPending as $r): ?>
                            <tr class="row-warn">
                                <td><?= htmlspecialchars($r['ReferenceNo']) ?></div>
                                <td><?= htmlspecialchars($r['ResidentName']) ?></div>
                                <td><?= htmlspecialchars($r['RequestType']) ?></div>
                                <td><span class="badge badge-<?= strtolower($r['Status']) ?>"><?= htmlspecialchars($r['Status']) ?></span></div>
                                <td><strong><?= (int)$r['HoursElapsed'] ?>h</strong></div>
                            </tr>
                        <?php endforeach; endif; ?>
                    </tbody>
                </table>
            ...
            <!-- ADDED: Completed Transactions Table -->
            <div class="card mt-4">
                <div class="card-header">
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        <?php if (empty($completedTransactions)): ?>
                            <tr><td colspan="8" class="empty-row">No completed transactions yet.</div></tr>
                        <?php else: foreach ($completedTransactions as $t): ?>
                            <tr>
                                <td><?= htmlspecialchars($t['ReferenceNo']) ?></div>
                                <td><?= htmlspecialchars($t['ResidentName']) ?></div>
                                <td><?= htmlspecialchars($t['RequestType']) ?></div>
                                <td><?= $t['ReleasedAt'] ? date('M d, Y h:i A', strtotime($t['ReleasedAt'])) : '—' ?></div>
                                <td>₱<?= number_format((float)$t['Amount'], 2) ?></div>
                                <td><?= htmlspecialchars($t['PaymentMethod']) ?></div>
                                <td><?= htmlspecialchars($t['ReceiptNo']) ?></div>
                                <td><?= htmlspecialchars($t['ReleasedBy'] ?? '—') ?></div>
                            </tr>
                        <?php endforeach; endif; ?>
                    </tbody>
                </table>
